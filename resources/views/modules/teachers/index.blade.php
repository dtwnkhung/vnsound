        ...
        columnDefs: [
          {
            ...
            render: function (data, type, row, meta) {
              var urlEdit = '{{ route("teachers.edit", ":id") }}';
              urlEdit = urlEdit.replace(':id', row.id);
              var urlDel = '{{ route("teachers.delete", ":id") }}';
              urlDel = urlDel.replace(':id', row.id);
              var urlChangePass = '{{ route("teachers.changePassword", ":id") }}';
              urlChangePass = urlChangePass.replace(':id', row.id);
              return (
                      ...
                      'Xóa</a>' +
                      '<a href="'+urlChangePass+'" class="dropdown-item">' +
                      feather.icons['edit'].toSvg({ class: 'mr-50 font-small-4' }) +
                      'Đổi mật khẩu</a>' +
                      '</div>' +
                      '</div>'
